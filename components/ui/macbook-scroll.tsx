...
export const MacbookScroll = ({
  ...
  useEffect(() => {
    const unsubscribe = scrollYProgress.on("change", (latest) => {
      setIsInteractive(latest >= 0.9)
    })
    return unsubscribe
  }, [scrollYProgress])

  const scaleX = useTransform(scrollYProgress, [0, 0.3, 0.9], [1.2, isMobile ? 1 : 1.5, isMobile ? 1 : 1.5])
  const scaleY = useTransform(scrollYProgress, [0, 0.3, 0.9], [0.6, isMobile ? 1 : 1.5, isMobile ? 1 : 1.5])
  const translate = useTransform(scrollYProgress, [0, 0.9, 1], [0, 0, 0])
  const rotate = useTransform(scrollYProgress, [0.1, 0.12, 0.3, 0.9], [-28, -28, 0, 0])
  ...
  return (
    <div
      ref={ref}
      className={`flex min-h-[250vh] shrink-0 scale-[0.35] transform flex-col items-center justify-start py-0 [perspective:800px] sm:scale-50 md:scale-100 md:py-80 transition-all duration-300 ${
    ...
    >
      <motion.h2
        ...
        className="mb-20 text-center text-3xl font-bold theme-text"
      >
        {title || (
          <span>
      ...
      <div className="relative -z-10 h-[22rem] w-[32rem] overflow-hidden rounded-2xl bg-gray-200 dark:bg-[#272729]">
        ...
        {isInteractive && (
          <div className="absolute top-4 right-4 z-50 px-3 py-1 bg-green-500 text-white text-sm rounded-full animate-pulse">
            Interactive Mode
          </div>
        )}
      </div>
    ...
    <div className="relative [perspective:800px]">
      ...
      >
        ...
        <img
          ...
          alt="aceternity logo"
          className={`absolute inset-0 h-full w-full rounded-lg object-cover object-left-top transition-all duration-300 ${
        ...
        {isInteractive && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 hover:opacity-100 transition-opacity duration-300">
            <div className="text-white text-center">
              <p className="text-lg font-semibold mb-2">Click to Explore</p>
              <p className="text-sm">Interactive content available</p>
            </div>
          </div>
        )}
      </motion.div>
