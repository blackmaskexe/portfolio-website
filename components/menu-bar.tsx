...
export function MenuBar({ currentTime, onControlCenterClick }: MenuBarProps) {
  ...
  const appleMenuItems = [
    ...
    { label: "Portfolio Settings...", action: () => alert("Portfolio settings coming soon!") },
    { label: "Recent Projects", action: () => alert("Check out my latest work on GitHub!") },
    { label: "View All Projects", action: () => alert("Opening full project gallery...") },
    { type: "separator" },
    { label: "Developer Contact", action: () => alert("Email: developer@example.com\nLinkedIn: /in/developer") },
    { label: "Switch to Simple Portfolio", action: () => alert("Switching to minimal portfolio view...") },
    { type: "separator" },
    { label: "Exit Portfolio", action: () => alert("Thanks for visiting my interactive portfolio!") },
  ]
    ...
    <div className="absolute top-0 left-0 right-0 h-6 bg-black/20 backdrop-blur-md border-b border-white/10 flex items-center justify-between px-4 text-white text-sm font-medium z-50">
      ...
      <div className="flex items-center space-x-4">
        <div className="relative">
          <button onClick={() => setShowAppleMenu(!showAppleMenu)} className="hover:bg-white/10 p-1 rounded">
